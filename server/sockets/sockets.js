...
const {
  ...
  logToClient,
  getBufferedLogs,
} = require("../utils/logger"); // import logger instance
...
const setupSockets = (server) => {
  ...
  (async () => {
    ...
    try {
      ...
      logToClient(
        "error",
        `Failed to Load Classes (${err.code})`,
        `Could not load classses from database. Check database connection.`
      );
    }
  })();
  ...
  io.on("connection", (socket) => {
    ...
    // Add aula
    // Servidor - socket handler
    socket.on("add-aula", async (data) => {
      try {
        const newId =
          schedule.length > 0 ? schedule[schedule.length - 1].Cod_Aula + 1 : 1;
        data.newAula.Cod_Aula = newId;

        console.log("Aula com dados completos:", data.newAula);
        ...
        socket.emit("add-aula-error", { message: "Erro ao adicionar aula." });
      }
    });

    // Remove aula
    socket.on("remove-aula", async (data) => {
      const aulaExists = schedule.some(
        (aula) => aula.Cod_Aula === data.codAula
      );
      if (aulaExists) {
        try {
          ...
          socket.emit("remove-aula-error", {
            message: "Erro ao remover aula do banco de dados.",
          });
        }
      } else {
    ...
    socket.on("update-aula", async (data) => {
      const aulaToUpdate = schedule.find(
        (aula) => aula.Cod_Aula === data.codAula
      );
      if (aulaToUpdate) {
        try {
          ...
          socket.emit("update-aula-error", {
            message: "Erro ao atualizar aula no banco de dados.",
          });
        }
      } else {
        console.log("Aula not found for update: ", data.codAula);
        socket.emit("update-aula-error", {
          message: "Esta aula não pode ser atualizada.",
        });
      }
    });
